"""
SPORTS VIDEO TRACKER V2 — CSRT + Kalman Trajectory Projection
=============================================================
Dependencies:
...
# If the target jumps further than this multiplier of its own size compared to 
# its projected momentum, we assume an ID Switch and reject the visual track.
...
class SportsTracker:
    ...
    def _init_tracking(self, frame: np.ndarray, bbox: tuple):
        ...
        # Reset Kalman to avoid dragging old momentum into new selection
        self.kalman = KalmanPredictor()
    ...
    # ── Re-detection strictly tied to projected trajectory ───────────────────
    def _redetect(self, frame: np.ndarray, pred_pos: tuple):
        ...
        # Force search ONLY along the projected trajectory to prevent ID switches
        if pred_pos and self.kalman_bbox_size:
            px, py   = pred_pos
            tw, th   = self.kalman_bbox_size
            margin   = max(tw, th) * 2  # Tight margin around physical projection
            fh, fw   = g_frame.shape
            ...
            return None # Don't globally search if we lose physical context
    ...
    # ── Kinematic Validation & Tracking Update ────────────────────────────────
    def _step(self, frame: np.ndarray):
        if self.track_state == self.TS_NONE:
            return None

        # 1. ALWAYS project the physical trajectory forward first
        ...
        # 3. Trajectory Integrity Check (Preventing Jumps & Direction Breaks)
        if ok and ov > 0.1 and self.track_state in [self.TS_TRACKING, self.TS_COASTING]:
            ...
            if pred_pos and self.kalman_bbox_size:
                ...
                # Measure spatial deviation from expected physics path
                dist = np.hypot(cx - px, cy - py)
                max_dist = max(w, h) * TRAJECTORY_TOLERANCE

                if dist > max_dist:
                    anomaly_detected = True
                    print(f"[ANOMALY] ID Switch Risk! Target jumped {dist:.1f}px off trajectory (Limit {max_dist:.1f}px)")

        # A: Good visual tracking AND passes physics checks
        if ok and ov > 0.6 and not anomaly_detected:
            self.bbox = raw
            cx, cy = self._center(raw)
        ...
        # B: Partially outside frame (Ignore anomalies here, object is clipping)
        if ok and 0.1 < ov <= 0.6 and not anomaly_detected:
            self.bbox = raw
        ...
        if pred_pos and self.kalman_bbox_size:
            ...
            self.trail.append((pred_pos[0], pred_pos[1])) # Draw projected trail
    ...
    def _draw_hud(self, frame: np.ndarray):
        ...
        label, color, blink = state_cfg.get(self.track_state, ("", (255,255,255), False))
        if not label: return
        if blink and (int(time.time() * 3) % 2 == 0): return   
    ...
    def run(self):
        print("=" * 55)
        print("  SPORTS TRACKER V2 (Trajectory Aware) ready")
