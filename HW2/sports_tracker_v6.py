"""
SPORTS VIDEO TRACKER V6 — Anchor Template + Color Drift Validation
==================================================================
Dependencies:
...
COLOR_DIST_THRESHOLD = 0.50  # Max allowed deviation from original color profile
...
CLR_KICK   = (255, 255,  0)  
CLR_ANCHOR = (180,  30, 255) # Pink/Purple for anchor re-detection
...
class SportsTracker:
    ...
    def _get_crop(self, frame: np.ndarray, bbox: tuple):
        x, y, w, h = [int(v) for v in bbox]
        fh, fw = frame.shape[:2]
        x1, y1 = max(0, x), max(0, y)
        x2, y2 = min(fw, x + w), min(fh, y + h)
    ...
    def _init_tracking(self, frame: np.ndarray, bbox: tuple):
        ...
        # Save V5 rolling template
        self._save_template(frame, bbox, force=True)
        
        # Save V6 Anchor Instance (Never overwritten)
        self.original_template = self._get_crop(frame, bbox)
        self.original_hist     = self._extract_color_hist(frame, bbox)
        
        self.trail.clear()
        self.kick_flash_timer = 0.0
        print(f"[TRACKER] Initialised  bbox={bbox}")

    def _is_open_grass(self, frame: np.ndarray, bbox: tuple):
        x, y, w, h = [int(v) for v in bbox]
        fh, fw = frame.shape[:2]
        margin_x = max(10, w)
        margin_y = max(10, h)
        ...
        ix1 = x - x1
        iy1 = y - y1
        mask[iy1:iy1+h, ix1:ix1+w] = 0
        
        total_halo_pixels = (x2 - x1) * (y2 - y1) - (w * h)
        if total_halo_pixels <= 0: return False
        
        green_ratio = cv2.countNonZero(mask) / total_halo_pixels
    ...
    # ── V6 RE-DETECTION (Strictly uses Original Anchor) ──────────────────────
    def _redetect_anchor(self, frame: np.ndarray, pred_pos: tuple):
        if self.original_template is None:
            return None
            
        g_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        g_tmpl  = cv2.cvtColor(self.original_template, cv2.COLOR_BGR2GRAY)

        if pred_pos and self.kalman_bbox_size:
            px, py   = pred_pos
            tw, th   = self.kalman_bbox_size
            margin   = max(tw, th) * 3  # Wider search margin for recovery
            fh, fw   = g_frame.shape
    ...
    def _step(self, frame: np.ndarray):
        ...
        # ── NEW IN V6: HARSH COLOR DRIFT VALIDATION ──
        drift_detected = False
        if ok and ov > 0.1:
            current_hist = self._extract_color_hist(frame, raw)
            if self.original_hist is not None and current_hist is not None:
                color_dist = cv2.compareHist(self.original_hist, current_hist, cv2.HISTCMP_BHATTACHARYYA)
                if color_dist > COLOR_DIST_THRESHOLD:
                    drift_detected = True
                    print(f"[DRIFT] Harsh color change! Dist={color_dist:.2f}. Rejecting visual track.")

        # If drift detected, we treat it as if the tracker failed to force a re-detection
        if drift_detected:
            ok = False
        ...
        # V5: Kick & Teleportation Detection
        if ok and ov > 0.1 and self.track_state in [self.TS_TRACKING, self.TS_COASTING]:
            ...
            if pred_pos and self.kalman_bbox_size:
                ...
                if dist_pred > max_pred_dist:
                    ...
                    max_kick_dist = max(w, h) * MAX_KICK_DISTANCE
                    
                    if dist_last < max_kick_dist:
                        is_kick_event = True
                    else:
        ...
        # C & D: Coasting / Re-Detection Block
        self.lost_count += 1
        k_box = None
        ...
        # V6: Try to re-detect the object using the ORIGINAL anchor template
        found = self._redetect_anchor(frame, pred_pos)
        if found:
            rb, conf = found
            print(f"[RECOVERED] Anchor instance found nearby! conf={conf:.2f}")
            self._init_tracking(frame, rb)
            self.track_state = self.TS_TRACKING
            # Temporarily flash purple to indicate anchor snap
            self.kick_flash_timer = time.time() + 0.1 
            return rb
    ...
    def _draw_bbox(self, frame: np.ndarray, bbox: tuple, state: str):
        ...
        # Color overrides for events
        if time.time() - self.kick_flash_timer < 0.5:
            if time.time() - self.kick_flash_timer > 0.4:
                color = CLR_ANCHOR # Hacky way to flash anchor snap color
            else:
                color = CLR_KICK
        else:
            color = {
    ...
    def _draw_hud(self, frame: np.ndarray):
        ...
        cv2.rectangle(frame, (0, 0), (190, 85), (0, 0, 0), -1)
        ...
        cv2.putText(frame, tmpl_txt, (8, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.50, tmpl_col, 1, cv2.LINE_AA)
    ...
    def run(self):
        print("=" * 55)
        print("  SPORTS TRACKER V6 (Anchor Template Validation)")
