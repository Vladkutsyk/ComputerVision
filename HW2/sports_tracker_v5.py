"""
SPORTS VIDEO TRACKER V5 — CSRT + Kalman + Grass Freezing + Kick Detection
=========================================================================
Dependencies:
    pip install opencv-contrib-python numpy

Usage:
    python sports_tracker_v5.py <video_file>
"""
...
TRAJECTORY_TOLERANCE = 1.15  # Jump multiplier relative to box size to trigger anomaly
MAX_KICK_DISTANCE    = 4.5   # Jump multiplier from LAST frame to classify as a valid "Kick"
...
CLR_PIP    = (0,   220, 220)
CLR_TRAIL  = (0,   180, 255)
CLR_FROZEN = (255,  50,  50) # Blue for frozen template
CLR_KICK   = (255, 255,  0)  # Cyan for kick flash
...
class SportsTracker:
    ...
    # ── NEW IN V5: ENVIRONMENT CHECK ─────────────────────────────────────────
    def _is_open_grass(self, frame: np.ndarray, bbox: tuple):
        """Checks if the area immediately surrounding the box is mostly green grass."""
        x, y, w, h = [int(v) for v in bbox]
        fh, fw = frame.shape[:2]
        
        # Create a search halo around the box (1.5x width/height)
        margin_x = max(10, w)
        margin_y = max(10, h)
        ...
        # Define football pitch green in HSV
        lower_green = np.array([35, 40, 40])
        upper_green = np.array([85, 255, 255])
        mask = cv2.inRange(hsv, lower_green, upper_green)
        
        # Black out the object itself so we only measure the surrounding halo
        ix1 = x - x1
        iy1 = y - y1
        ...
        return green_ratio > 0.40  # If more than 40% of the halo is grass

    def _save_template(self, frame: np.ndarray, bbox: tuple, force=False):
        """Saves template ONLY if on open grass, to prevent learning a player's shoe."""
    ...
    def _step(self, frame: np.ndarray):
        ...
        # ── NEW IN V5: KICK DETECTION (Vector Snap) ──
            ...
            if pred_pos and self.kalman_bbox_size:
                ...
                if dist_pred > max_pred_dist:
                    ...
                    if dist_last < max_kick_dist:
                        is_kick_event = True
                        print(f"[KICK DETECTED] Sharp vector snap! (Off-path by {dist_pred:.1f}px)")
                    else:
                        anomaly_detected = True
                        print(f"[ANOMALY] Teleportation detected. Dist={dist_pred:.1f}px. Coasting...")
        ...
        if ok and not anomaly_detected:
            ...
            # Reset Kalman momentum if kicked
            if is_kick_event:
            ...
            # Attempt to update visual template (will freeze if entangled)
        ...
        if found:
            rb, conf = found
            print(f"[RECOVERED] Target found on trajectory. conf={conf:.2f}")
    ...
    def run(self):
        print("=" * 55)
        print("  SPORTS TRACKER V5 (V4 + Grass Freezing + Kick Logic)")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python sports_tracker_v5.py <video_file>")
        sys.exit(1)
    path = sys.argv[1]
