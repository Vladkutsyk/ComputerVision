...
class SportsTracker:
    ...
    def _step(self, frame: np.ndarray):
        ...
        # If the object is too grass-like or absent for too long, stop.
        if pred_pos:
            if self._reference_confidence(frame, k_box if k_box else self.bbox) < 0.20:
                self._stop_tracking("Target is likely on grass / background, stopping tracking.")
                return self.bbox
